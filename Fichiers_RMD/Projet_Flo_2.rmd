...
```{r}
#Model 65% avec variables confort *** et tailmng * 
DB_test_model_1<-DB_pers_socio_1%>%
  mutate(Comfort_social=case_when(DB_pers_socio_1$csp %in%c("HIGH_STATUT")&DB_pers_socio_1$ABO_TC %in% c("OUI_GRATUIT","OUI_PAYANT") & DB_pers_socio_1$zonetrav %in% c("CENTRE_VILLE") &DB_pers_socio_1$car_ownership %in%c("OUI") ~ 1,
              DB_pers_socio_1$csp %in% c("LOW_STATUT")   & DB_pers_socio_1$zonetrav %in% c("HORS_GRENOBLE") & DB_pers_socio_1$car_ownership %in% c("NON") ~3,
...
splitIndex=createDataPartition(DB_test_model_1$freqtcu, p = 0.8, list = FALSE, times = 1)
train=DB_test_model_1[ splitIndex,]
test=DB_test_model_1[-splitIndex,]
model_glm_test =glm(formula = freqtcu ~  tailmng + Comfort_social, data = train, family = "binomial")

summary(model_glm_test)
```

```{r pred-mod-reg-log}
predictions <- predict(model_glm_test, newdata = test, type = "response")
pred_class<-factor(ifelse(predictions>0.5,1,0))
#pred_class<-factor(predictions, levels = levels(test$freqtcu))
#prediction_freqtcu_test= factor(predictions,levels=levels(test$freqtcu))
...
```{r}
confusionMatrix(pred_class,test$freqtcu)
```
