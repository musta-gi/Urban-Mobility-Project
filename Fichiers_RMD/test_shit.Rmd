...
## Restriction géographique de la base de données
...
```{r}
DB_cat_soc <- New_DB %>% select(id_pers, freqtcu, OCCU1) %>% distinct() %>% group_by(OCCU1)
DB_cat_soc <- na.omit(DB_cat_soc)

DB_cat_soc_use_tc <- DB_cat_soc %>% filter(freqtcu == 1) %>% count(OCCU1)

DB_cat_soc_dont_use_tc <- DB_cat_soc %>% filter(freqtcu == 0) %>% count(OCCU1)

head(DB_cat_soc_use_tc)
```
